...
While ideally opulist attitude scale captures populist ideation across different ideological predispositions (see the critique of @castanho2020 of populist item scales including nativist items), 'the people' is often construed in ethnic, cultural, or even racial terms on the (far) right and conceived of in more civic or economic terms on the (far) left. 
Using a wording experiment in Germany (`N=7,034`), we examine measurement invariance in multiple ways.
...
```
```{r results-h1,out.width = "99%", fig.align = 'center', fig.cap = "\\label{fig:results-h1}Differences in Ethnic and Civic Conceptions of the People"}
knitr::include_graphics(here("report/figures", "h1-extra-1.png"))
```
...
Does an ethnic and civic framing of the items `POP1` and `POP2` affect the measurement invariance assumption of the latent construct of populist attitudes? 
...
Moreover, when testing metric and scalar invariance -- i.e. is the magnitude of loadings similar accross groups or are the magnitude of loadings and the intercept difference accross groups -- we find statistical significant Chi-square differences for the metric invariance and scalar invariance models, shown in Table \ref{tab:mi-cfa}.
This indicates that the assumption that the different conceptions of the people function similar is not met. 
...
This scaling model shows that `POP1` and `POP2` contribute slightly more information to the latent construct when the items are ethnically phrased, as opposed to a civic framing (see **Figure XX** and **XX** in the Appendix). 
While the means of the scales with the different conceptions of the people are not different, testing configural, metric and scalar invariance -- see Table \ref{tab:mi-irt} -- we see that the equal slope assumption (metric invariance) as well as one form of scalar invariance (i.e. the one with free means) are not met.
...
dat <- a1[2,] %>% 
  add_case(a2[2,]) %>% 
  add_case(a3[2,]) %>% 
  mutate(`Equivalence Tests` = c("Equal Slopes",
...
Moreover, we have created an additive scale (see Table **XX** in the OA), as well as constructed a scale using the approach of @wuttke2020whole -- i.e. using the lowest value on one of the items that construe the latent construct.
...
An ethnic conception of the people, overall, yielded more agreement with the items in question -- but not with the latent construct. 
