...
thanks: "Replication files are available on the author's Github account (https://anonymous.4open.science/r/wording-experiment-populist-attitudes-F3B2/README.md). **Current version**: `r format(Sys.time(), '%B %d, %Y')`; **Corresponding author**: Maurits Meijers"
author:
...
While in some  cultural contexts, there is no strong semantic difference between 'the people' and 'citizens' (e.g. in the English-speaking world), in other contexts, such as the Germanic one, 'the people' (i.e., _das Volk_) has a strong ethnic connotation. 
...
After World War I, the terms _Volk_ and _Volksdeutsche_ were used to describe a ethnically and culturally homogeneous German diaspora, who were citizens of different states after the Versailles Treaty of 1919 [@gosewinkel2003]. 
...
Yet one item in the @akkerman2014 scale contrasts 'the citizen' with 'politicians' (`POP4`) - an item that nonetheless contributes significantly to the latent construct of populism [@vanhauwaert2020]. 
What is more, 'the people' has been translated differently in different translations of the populist attitude scale. 
...
In Italian, 'the people' is sometimes referred to as _il popolo_, and sometimes as _le persone_ [@castanho2020]. 

This begs the question whether a civic or ethnic conception of the people elicits differences in measurement of populist attitudes. 
On the (far) right, 'the people' is often construed in ethnic, cultural, or even racial terms. 
On the left, 'the people' is often conceived of in more civic or economic terms. 
Ideally, however, populist attitude scale captures populist ideation across different ideological predispositions (see the critique of @castanho2020 of populist item scales including nativist items). 
Using a wording experiment in Germany (`N=7,034`), we examine whether an ethnic or a civic conception of the people affects respondents' agreement with the two items of the populist attitude scale that directly juxtapose 'the people' with the elites.[^1] 
We further examine whether far right voters and respondents with a exclusive national identity respond differently to ethnic conceptions of the people. In addition, we examine whether the differences found affect the overall latent construct of populist attitudes (**EXPAND/SPECIFY LATER**). 
...
[^1]: All analytical material for replication can be found in our online Research Compendium: https://anonymous.4open.science/r/wording-experiment-populist-attitudes-F3B2/README.md (anonymized for the review process).

# Civic versus Ethnic Conception of "the People"
...
The sample is from an online panel provided by the survey company Respondi -- for an overview of all variables in the study, as well as the descriptive information, see Online Appendix (OA pp. A2-A6). 
...
This large sample size allows us to detect also small effects of question wording differences.[^2]

[^2]: A power analysis is conducted and reported on our [Online Research Compendium](https://anonymous.4open.science/r/wording-experiment-populist-attitudes-F3B2/README.md). 
...
```
```{r results-h1,out.width = "99%", fig.align = 'center', fig.cap = "\\label{fig:results-h1}Results: H1"}
knitr::include_graphics(here::here("report/figures", "h1-extra-1.png"))
```
...
The right-hand panel of Figure \ref{fig:results-h1} indicates that the non-effect for `POP1` is driven by the fact that for younger age groups (younger than 50 years old) there is no difference for the civic and ethnic conception of the people, as indicated by the blue dots and whiskers in Figure \ref{fig:results-h1}.[^4]
For the older population (50 years old and more), the right-hand panel of Figure \ref{fig:results-h1} demonstrates using an OLS regression, including an interaction between treatment and age groups, that for both `POP1` and `POP2`, people receiving the ethnic treatment report higher levels of agreement with the items compared to people receiving the civic treatment.
The effect size is still small in substantive terms: between a 0.1 and 0.3 increase on a 5-point scale.

[^4]: Our [Online Research Compendium](https://anonymous.4open.science/r/wording-experiment-populist-attitudes-F3B2/README.md) demonstrates that there are no other socio-demographic variables that mediate the effect. 

How does the effect of an ethnic conception of the people vis-à-vis a civic conception vary across respondents? 
The left-hand panel of Figure \ref{fig:results-h2} shows the results of an OLS regression analysis[^5] with the an interaction between the treatment and a variable measuring whether or not the respondent voted for the far right party _Alternative für Deutschland_ -- for more details see Table A.6 in the OA).
...
[^5]: As specified in the Pre-Analysis Plan, we only include unbalanced covariates. OA p. A-7 demonstrates the balance checks, demonstrating that only political interest and income are unbalanced, and therefore included as a covariate.
...
So how does an ethnic and civic framing of the items `POP1` and `POP2` affect the latent construct of populism? 
A graded Item Response Theory (IRT) analysis [see also @vanhauwaert2020] moreover shows that `POP1` and `POP2` contribute slightly more information to the latent construct when the items are ethnically phrased, as opposed to a civic framing (see **Figure XX** and **XX** in the Appendix). 
Overall, however, the overall level of information yielded by the two latent constructs of populism are similar. 
This is not surprising, given that we varied the wording of `the people' only in two items of the full @akkerman2014 item battery. 
Therefore, also overall difference between a latent construct of populism with two ethnically framed items and a latent construct with two civically framed items is small.

We find no statistically significant difference between the two constructs estimated using Confirmatory Factor Analysis (see Table A.9 in the OA).
...
Yet, when looking at the additive scale (see Table **XX** in the OA), we see that the scale using the civic conception of the people has a statistically higher mean value compared to the ethnic conception.
The same holds for constructing a scale using the approach of @wuttke2020whole -- i.e. using the lowest value on one of the items that construe the latent construct.
...
```{r "scale", fig.align="center", fig.height=8, fig.width=8, eval = FALSE}
...
kbl(m_cfa, booktabs =T, caption = "\\label{tab:CFA}Confirmatory Factor Analysis - Fit Statistics") %>%
  kable_styling(latex_options = c("striped", "hold_position"),
                full_width = F, fixed_thead = T, position = "center") %>%
...
# Conclusion
Existing populist attitudes scales have been inconsistent in using 'ethnic' or 'civic' conceptions of the people. 
Using a wording experiment we examined whether German respondents presented with an ethnic framing of 'the people' (_das Volk_) answered key populist attitude items differently than respondents who recevied an ethnic framing of `the people' as citizens (_Bürger_). 

We find that there are statistically significant differences between the two wordings. 
An ethnic conception of the people, overall, yielded more agreement with the items in question. 
This is effect is particularly pronounced for voters of the far right, as well as for older voters. 
The difference between an ethnic and a civic conception of the people was also only significant for respondents who adhered a moderate to high level of exclusive national identity. 
This finding is highly suggestive of the 'ethnic' interpretation of `the people' -- at least in the German language.

Nevertheless, we found that a civic or an ethnic wording of items referring to 'the people' makes little difference in a latent construct that also comprises other items that were kept constant. 
Yet, while this might make little difference in a latent construct that also comprises items that do not solely rely on items that juxtapose 'the people' with the elite, it is likely to be more consequential in survey batteries that predominantly include items contrast 'the people' with elites to measure populist attitudes [for an overview of different batteries, see @castanho2020]. 
...
This type of differential item interpretation might therefore lead to measurement invariance when used in cross-national research [@castanho2020]. 
...
One can argue theoretically that 'the people' captures the meaning of the concept of populism better than 'citizens'. 
...
In addition, our results suggest that an ethnic framing of the citizens runs the, albeit small, risk overestimating the degree of populist attitudes among more nativist citizens. 
To avoid priming respondents with an ethnic conception of the people, future studies should consider using a civic conception of ''citizens vs. the elite" when measuring populist attitudes.
